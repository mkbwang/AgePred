...
#SBATCH --job-name=geneexp_deconvolution
...
Rscript --vanilla geneexp_train_deconv_type1.R -s ${SLURM_ARRAY_TASK_ID}
echo "Running deconvolution with sum to one constraint and smoothness penalty"
Rscript --vanilla geneexp_train_deconv_type2.R -s ${SLURM_ARRAY_TASK_ID}
echo "Running deconvolution without sum to one constraint but with sparseness and smoothness penalty"
Rscript --vanilla geneexp_train_deconv_type3.R -s ${SLURM_ARRAY_TASK_ID}
