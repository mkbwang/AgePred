...
for (j in 1:length(studies)){
    ...
    deconv_type2 <- read.xlsx(file.path(deconv_folder,
                              ...
                              sheet="Prediction",
                              rowNames=TRUE)
    methylation_result_df$Deconv2_original[j] <- mean(abs(deconv_type2$age - deconv_type2$Prediction_mean_original))
    methylation_result_df$Deconv2_normalized[j] <- mean(abs(deconv_type2$age - deconv_type2$Prediction_mean_normalized))
    ...
    methylation_result_df$Deconv3_original[j] <- mean(abs(deconv_type3$age - deconv_type3$Prediction_mean_original))
    methylation_result_df$Deconv3_normalized[j] <- mean(abs(deconv_type3$age - deconv_type3$Prediction_mean_normalized))
    ...
    png_name <- sprintf("methylation_%s.png", study)
    svg_name <- sprintf("methylation_%s.svg", study)
